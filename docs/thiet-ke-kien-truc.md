...
│                             ▼                                    │
│  ┌─────────────────────────────────────────────────────────┐    │
│  │  PYTHON RUNTIME  (.venv — Python 3.11+)                  │    │
│  │                                                          │    │
...
**Outcome:** Gõ `resolve_model("account.move", "17.0")` trong VS Code → Claude trả lời đúng toàn bộ chain từ `odoo` → `acme_addons` → `acme_enterprise`, không hallucinate.
