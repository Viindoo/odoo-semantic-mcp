...
| `standard_profile_N` | Chỉ Viindoo addons (`acme_addons`, `acme_enterprise` v10+, `branding` v13+) |
| `internal_profile_N` | Chỉ internal repos (`acme_infra`, `acme_infra_common`, `themes` v17-only, `acme_api`) |

Để dùng **Standard Viindoo** cho 1 version, admin index CẢ `odoo_N` VÀ
`standard_profile_N`. Để dùng **Viindoo Internal**, index thêm
`internal_profile_N`. MCP queries (`resolve_model`, `find_examples`, …) tự
combine vì chúng scope theo `odoo_version`, không phải `profile_name` —
nghĩa là `Viindoo/odoo @ 17.0` (thuộc `odoo_17`) và `acme_addons @ 17.0`
(thuộc `standard_profile_17`) đều xuất hiện chung khi query version `17.0`.
...
Kỳ vọng: thấy 26 seeded profiles `odoo_8` ... `odoo_19`, `standard_profile_8` ...
`standard_profile_19`, `internal_profile_17`, `internal_profile_18` — kèm
...
| `standard_profile_8`, `standard_profile_9` | **1** each (`acme_addons`) |
| `standard_profile_10` … `standard_profile_12` | **2** each (+`acme_enterprise`) |
| `standard_profile_13` … `standard_profile_19` | **3** each (+`branding`) |
| `internal_profile_17` | **4** (`acme_infra`, `acme_infra_common`, `themes`, `acme_api`) |
| `internal_profile_18` | **3** (no `themes` — max branch là 17.0) |
...
|---|---|---|
| Profile name đã có (vd `internal_profile_17` manual) | Seed skip (`ON CONFLICT (name) DO NOTHING`) | Admin data win. Review trùng lặp qua `python -m src.manager list`. |
...
> with `odoo_`, `standard_profile_`, or `internal_profile_` — including
> admin-created profiles that happen to share those prefixes (e.g.
> `odoo_custom_client_v18`, `standard_profile_qa`). Backup PostgreSQL trước khi
