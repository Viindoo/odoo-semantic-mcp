...
> nhận master data mặc định (26 profiles + 68 repos seeded tự động).
> Xem [`docs/deploy.md`](../deploy.md) cho fresh install.

## Mục Tiêu

`python -m src.db.migrate` từ version này trở đi tự động seed 26 master data
profiles + 68 repos rows vào PostgreSQL:

- **Odoo CE v8 → v19** (12 profiles, mỗi profile chỉ 1 repo `Viindoo/odoo`).
- **Standard Viindoo v8 → v19** (12 profiles, 2–4 repos tùy version).
- **Viindoo Internal v17 và v18** (2 profiles, 7–8 repos tùy version).
...
disk + bandwidth khi chưa cần dùng hết 26 versions).
...
✓ Seeded master data: 26 profiles (X new, Y unchanged), 68 repos (P new, Q unchanged)
```
...
| Profile tier | Repo count |
|---|---|
| `odoo_8` … `odoo_19` | **1** each |
| `standard_viindoo_8`, `standard_viindoo_9` | **2** each |
| `standard_viindoo_10` … `standard_viindoo_12` | **3** each |
| `standard_viindoo_13` … `standard_viindoo_19` | **4** each |
| `viindoo_internal_17` | **8** |
| `viindoo_internal_18` | **7** (no `themes` — branch không có ở v18) |
